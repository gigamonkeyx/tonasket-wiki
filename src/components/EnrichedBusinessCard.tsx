...
const EnrichedBusinessCard: React.FC<EnrichedBusinessCardProps> = ({ business, featured = false }) => {
  ...
  // Format hours string to display nicely
  const formatHours = (hours?: string) => {
    if (!hours) return 'Hours not available';

    // If hours is already in a nice format, return it
    if (hours.includes('\n') || hours.includes('<br>')) {
      return hours.replace(/<br>/g, '\n');
    }

    // Try to format comma-separated hours
    if (hours.includes(',')) {
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="mb-4">
          <div
            className={`text-gray-700 dark:text-gray-300 ${isDescriptionExpanded ? '' : 'line-clamp-3'} overflow-y-auto`}
          ...
          <div className="mb-3">
            <div className="flex items-center">
              ...
              <span className={`text-sm font-medium ${
                ...
                {business.licenseStatus} License
              </span>
            </div>
            {business.licenseType && (
              <div className="text-xs text-gray-600 dark:text-gray-400 ml-5 mt-1">
                {business.licenseType}
              </div>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {business.tags && business.tags.slice(0, 3).map((tag, index) => (
        ...
        {business.services && business.services.length > 0 && (
          <div className="mb-4">
            <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-1">Services</h4>
            <div className="flex flex-wrap gap-1">
              {business.services.slice(0, 4).map((service, index) => (
                <span key={index} className="inline-block bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs px-2 py-1 rounded">
                  {service}
                </span>
              ))}
              {business.services.length > 4 && (
                <span className="inline-block bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs px-2 py-1 rounded">
        ...
        {business.hours && (
          <div className="mb-4">
            <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-1">Hours</h4>
            ...
            <div className="flex gap-3">
              ...
              {business.licenseNumber && (
                <a
                  ...
                  rel="noopener noreferrer"
                  className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 text-sm font-medium flex items-center"
                  aria-label={`View ${business.name} license information (opens in new tab)`}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
...
export default EnrichedBusinessCard;
